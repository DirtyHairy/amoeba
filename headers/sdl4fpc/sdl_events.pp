...
{$PACKRECORDS 4}
  ...
  { Predefined event masks }
    ...
    { Polls for currently pending events, and returns True if there are any pending
       events, or False if there are none available.  If 'event' is not nil, the next
       event is removed from the queue and stored in that area.
      }
    function SDL_PollEvent(event:pSDL_Event): LongBool;cdecl;

    { Waits indefinitely for the next available event, returning True,
       or False if there
       was an error while waiting for events.  If 'event' is not nil, the next
       event is removed from the queue and stored in that area.
      }
    function SDL_WaitEvent(event:pSDL_Event): LongBool;cdecl;
  ...
  implementation
    ...
    function SDL_PollEvent(event:pSDL_Event):longBool;cdecl;external 'SDL';

    function SDL_WaitEvent(event:pSDL_Event):longBool;cdecl;external 'SDL';
